...
void op_cmp_reg(svm_t *svm) {
  ...
  if (svm->registers[reg1].type == svm->registers[reg2].type)
  {
    ...
    {
      if (strcmp
        (svm->registers[reg1].value.string,
      ...
      */
      void op_is_number(svm_t *svm) {
        ...
        if (svm->registers[reg].type == NUMBER)
          svm->flags.z = 1;
        else
          svm->flags.z = 0;
      ...
      */
      void op_peek(svm_t *svm)
      {
        ...
        if (getenv("DEBUG") != NULL)
        printf
        ("LOAD_FROM_RAM(Register:%d will contain values of address %04X)\n",
        reg, addr);
